...
void find_M(const int& size, const vector<int>& combs, const long long& combs_product, const long long& acc){
    for (int i = size; i < combs.size(); ++i) {
        if (acc * combs[i] <= Mmax){
            ...
            find_M(i, combs, combs_product, M);
        }else{
            break;
        }
    }
